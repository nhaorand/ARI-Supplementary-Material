...
import static wtune.superopt.fragment.FragmentUtils.*;
...
class FragmentImpl implements Fragment {
  ...
  private static void serializeTree(Op root, List<String> nodes_list, List<List<Integer>> adjacency_list) {
    List<Integer> cur_adjacency_list = new ArrayList<>();
    nodes_list.add(root.kind().text());
    adjacency_list.add(cur_adjacency_list);
    for (Op op : root.predecessors()) {
      cur_adjacency_list.add(nodes_list.size());
      serializeTree(op, nodes_list, adjacency_list);
    }
  }

  public String toTreeListStr() {
    List<String> nodesList = new ArrayList<>();
    List<List<Integer>> adjacencyList = new ArrayList<>();
    serializeTree(root, nodesList, adjacencyList);
    return nodesList.toString() + adjacencyList.toString();
  }
